...
export const vpcs = [
  ...
  },
  {
    ...
    description:
      'Research and experimental projects - applying configuration changes',
  ...
  },
  {
    ...
    resources: [{ type: 'VM', name: 'api-gateway', count: 1 }],
  },
  {
    id: 'vpc-10',
    name: 'legacy-vpc',
...
export const snapshots = [
  ...
  },
  {
    ...
    description:
      'Primary snapshot of logs storage volume - scheduled for deletion',
    policy: {
      enabled: false,
...
export const canDeletePrimarySnapshot = (snapshotId: string) => {
  ...
  if (snapshot.type !== 'Primary') {
    return true;
  }

  // Check if there are other Primary snapshots for the same volume/VM
  const otherPrimarySnapshots = snapshots.filter(
    s =>
      s.id !== snapshotId &&
      s.type === 'Primary' &&
...
export const addSubnetConnection = (
  subnetId: string,
  targetSubnetId: string
) => {
  // Add connection both ways
  let sourceConnection = subnetConnections.find(
    conn => conn.subnetId === subnetId
  );
  if (!sourceConnection) {
  ...
  let targetConnection = subnetConnections.find(
    conn => conn.subnetId === targetSubnetId
  );
  if (!targetConnection) {
...
export const removeSubnetConnection = (
  subnetId: string,
  targetSubnetId: string
) => {
  // Remove connection both ways
  const sourceConnection = subnetConnections.find(
    conn => conn.subnetId === subnetId
  );
  if (sourceConnection) {
    sourceConnection.connectedSubnets =
      sourceConnection.connectedSubnets.filter(id => id !== targetSubnetId);
  }

  const targetConnection = subnetConnections.find(
    conn => conn.subnetId === targetSubnetId
  );
  if (targetConnection) {
    targetConnection.connectedSubnets =
      targetConnection.connectedSubnets.filter(id => id !== subnetId);
  }
};
...
export const vmInstances = [
  {
    ...
    description:
      'Main production web server handling customer traffic and API requests',
    tags: [
  ...
  },
  {
    ...
    description:
      'High-performance GPU instance for machine learning model training and inference',
    tags: [
  ...
  },
  {
    ...
    deleteProtection: true,
    description:
      'Primary PostgreSQL database server with automated backups and high availability',
    tags: [
      { key: 'Environment', value: 'Production' },
  ...
  },
  {
    ...
    deleteProtection: false,
    description:
      'Development environment for testing new features and bug fixes',
    tags: [
      { key: 'Environment', value: 'Development' },
...
export const vmSecurityGroups = [
  {
    ...
    egressRules: [{ protocol: 'All', port: 'All', destination: '0.0.0.0/0' }],
  },
  {
    id: 'sg-002',
    name: 'database-sg',
...
export const attachSecurityGroup = async (
  vmId: string,
...
export const detachSecurityGroup = async (
  vmId: string,
  securityGroupId: string
): Promise<SecurityGroup | null> => {
  const securityGroup = vmSecurityGroups.find(sg => sg.id === securityGroupId);
  if (securityGroup && securityGroup.attachedTo.includes(vmId)) {
    securityGroup.attachedTo = securityGroup.attachedTo.filter(
      id => id !== vmId
    );
    return securityGroup;
  }
...
export const attachReservedIP = async (
  vmId: string,
  ipId: string
...
export const attachRandomIP = async (
  vmId: string
): Promise<PublicIP | null> => {
  const availableIP = publicIPs.find(
    p => p.status === 'available' && p.type === 'random'
  );
  if (availableIP) {
...
export const models: Model[] = [
  // TEXT MODELS
  {
    ...
    type: 'text',
    description:
      'Large-scale GPT model with 120B parameters for high-quality text generation',
    license: 'MIT',
    capabilities: [
      'text-generation',
      'summarization',
  ...
  },
  {
    ...
    type: 'text',
    description:
      'Efficient GPT model with 20B parameters optimized for speed and performance',
    license: 'Apache 2.0',
    capabilities: [
      'text-generation',
      'summarization',
  ...
  },
  {
    ...
    type: 'text',
    description:
      "Anthropic's most capable model for complex reasoning and analysis",
    license: 'Commercial',
    capabilities: [
      'text-generation',
      'reasoning',
      'complex-writing-conversations',
  ...
  },
  {
    ...
    type: 'text',
    description:
      'Mixture of experts model with excellent efficiency and performance',
    license: 'Apache 2.0',
  ...
  // EMBEDDING MODELS
  {
    ...
    type: 'embedding',
    description:
      'Most capable embedding model for semantic search and RAG applications',
    license: 'Commercial',
  ...
  },
  {
    ...
    type: 'embedding',
    description:
      'Multilingual embedding model with strong performance across languages',
    license: 'MIT',
    capabilities: ['context-rag', 'summarization'],
    pricing: {
      inputTokens: 0.008,
  ...
  },
  {
    ...
    type: 'audio',
    description:
      'Text-to-music generation model for creating high-quality audio',
    license: 'Research',
  ...
  },
  {
    ...
    type: 'audio',
    description:
      'Text-to-speech model with natural voice generation capabilities',
    license: 'MIT',
    capabilities: ['audio-generation'],
    pricing: {
      inputTokens: 0.08,
  ...
  // VISION MODELS
  {
    ...
    type: 'vision',
    description:
      'Multimodal model capable of understanding and reasoning about images',
    license: 'Commercial',
  ...
  },
  {
    ...
    type: 'vision',
    description:
      'Advanced vision model with superior image analysis capabilities',
    license: 'Commercial',
    capabilities: [
  ...
  },
  {
    ...
    type: 'vision',
    description:
      'Advanced text-to-image generation model with high fidelity output',
    license: 'Commercial',
  ...
  },
  {
    ...
    type: 'vision',
    description:
      'High-resolution image generation model with artistic capabilities',
    license: 'OpenRAIL',
    capabilities: ['image-generation'],
    pricing: {
      inputTokens: 0.02,
...
export const keyManagementServices: KeyManagementService[] = [
  {
    ...
    description:
      'Primary encryption key for production workloads and sensitive data',
    keyType: 'Symmetric',
    algorithm: 'AES_256',
    status: 'active',
    region: 'us-east-1',
  ...
  {
    keyId:
      'arn:aws:kms:us-east-1:123456789012:key/2345bcde-23bc-45de-67fa-2345678901bc',
  ...
  {
    keyId:
      'arn:aws:kms:us-west-2:123456789012:key/3456cdef-34cd-56ef-78gb-3456789012cd',
  ...
  {
    keyId:
      'arn:aws:kms:us-east-1:123456789012:key/4567defa-45de-67fa-89hc-4567890123de',
  ...
  {
    keyId:
      'arn:aws:kms:us-west-1:123456789012:key/5678efab-56ef-78ab-90id-5678901234ef',
  ...
  {
    keyId:
      'arn:aws:kms:us-east-2:123456789012:key/6789fabc-67fa-89bc-01je-6789012345fa',
  ...
  {
    keyId:
      'arn:aws:kms:us-east-1:123456789012:key/7890abcd-78ab-90cd-12kf-7890123456ab',
  ...
  {
    keyId:
      'arn:aws:kms:eu-west-1:123456789012:key/8901bcde-89bc-01de-23lg-8901234567bc',
  ...
  {
    keyId:
      'arn:aws:kms:ap-southeast-1:123456789012:key/9012cdef-90cd-12ef-34mh-9012345678cd',
  ...
  {
    keyId:
      'arn:aws:kms:us-east-1:123456789012:key/0123defa-01de-23fa-45ni-0123456789de',
    keyAlias: 'compliance-audit-key',
    description:
      'Dedicated key for compliance data encryption and audit trails',
    keyType: 'Symmetric',
    algorithm: 'AES_256',
    status: 'active',
    region: 'us-east-1',
