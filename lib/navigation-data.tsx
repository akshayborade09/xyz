...
export function getNavigationItemsForSection(section: string): NavItem[] {
  const sectionMap: Record<string, NavItem[]> = {
    infrastructure:
      navigationStructure.find(
        category => category.title === 'CORE INFRASTRUCTURE'
      )?.items || [],
    'ai-studio':
      navigationStructure.find(category => category.title === 'AI STUDIO')
        ?.items || [],
    'ola-maps':
      navigationStructure.find(category => category.title === 'OLA MAPS')
        ?.items || [],
    administration:
      navigationStructure.find(category => category.title === 'ADMINISTRATION')
        ?.items || [],
    'developer-tools':
      navigationStructure.find(category => category.title === 'DEVELOPER TOOLS')
        ?.items || [],
    'help-resources':
      navigationStructure.find(
        category => category.title === 'HELP & RESOURCES'
      )?.items || [],
    home:
      navigationStructure.find(category => category.title === '')?.items || [],
  };
