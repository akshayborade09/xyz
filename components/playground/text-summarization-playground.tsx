...
interface TextSummarizationPlaygroundProps {
  model: {
    name: string;
    provider: string;
    license: string;
...
export function TextSummarizationPlayground({
  model,
  selectedModel,
  modelData,
  onModelChange,
}: TextSummarizationPlaygroundProps) {
  ...
  const handleSummarize = () => {
    ...
    setTimeout(() => {
      ...
      const output = `Summarization Result\n\n` +
        `Input Preview:\n${inputText.slice(0, 200)}${inputText.length > 200 ? '…' : ''}\n\n` +
        `Result:\n- This is a concise summary of the provided text (mock).`;
      setSummaryText(output);
      setIsSummarizing(false);
      toast({ title: 'Summary generated', description: 'Document summarization complete.' });
    }, 1500);
  };
            ...
            <div className='space-y-4'>
              {!summaryText ? (
                <div className='flex flex-col items-center justify-center py-20 text-center'>
                  ...
                  <p className='text-gray-500 text-sm'>Summary will appear here</p>
                  <p className='text-gray-400 text-xs mt-2'>Enter text below and click "Summarize"</p>
                </div>
              ) : (
                <div className='space-y-4'>
                  <div className='flex items-center justify-between'>
                    <h3 className='text-sm font-semibold text-gray-700'>Summarization Result</h3>
                ...
                <motion.div className='relative w-full bg-white rounded-[24px] p-6 shadow-[0_2px_8px_0_rgba(0,0,0,0.08)]' animate={{ boxShadow: isInputFocused || inputText ? '0 8px 32px 0 rgba(0,0,0,0.16)' : '0 2px 8px 0 rgba(0,0,0,0.08)' }} transition={{ type: 'spring', stiffness: 120, damping: 18 }}>
                  <div className='space-y-4'>
                    <div className='flex items-start gap-3'>
                      ...
                      <Button onClick={handleSummarize} disabled={!inputText.trim() || isSummarizing} className='px-8'>
                        {isSummarizing ? (
                          <>
