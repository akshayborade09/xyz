...
import { Copy, RotateCcw, Languages } from 'lucide-react';

interface TextTranslationPlaygroundProps {
  model: {
    name: string;
    provider: string;
    license: string;
...
export function TextTranslationPlayground({
  model,
  selectedModel,
  modelData,
  onModelChange,
}: TextTranslationPlaygroundProps) {
  const { toast } = useToast();
  const [inputText, setInputText] = useState('');
  const [translatedText, setTranslatedText] = useState<string | null>(null);
  const [isTranslating, setIsTranslating] = useState(false);
  const [selectedSourceLanguage, setSelectedSourceLanguage] = useState('english');
  const [selectedTargetLanguage, setSelectedTargetLanguage] = useState('hindi');
  ...
  const handleTranslate = () => {
    ...
    setTimeout(() => {
      ...
      // Simple mock translation output
      const output = `Translated (${selectedSourceLanguage} → ${selectedTargetLanguage})\n\n${inputText}`;
      setTranslatedText(output);
      setIsTranslating(false);

      toast({ title: 'Translation generated', description: 'Your translated text is ready.' });
    }, 1500);
  };
            ...
            <div className='space-y-4'>
              {!translatedText ? (
                <div className='flex flex-col items-center justify-center py-20 text-center'>
                  ...
                  <p className='text-gray-500 text-sm'>Your translated text will appear here</p>
                  <p className='text-gray-400 text-xs mt-2'>Enter text below and click "Translate"</p>
                </div>
              ) : (
                <div className='space-y-4'>
                  <div className='flex items-center justify-between'>
                    <h3 className='text-sm font-semibold text-gray-700'>Translated Text</h3>
                ...
                >
                  <div className='space-y-4'>
                    ...
                    <div className='flex items-end gap-3'>
                      ...
                      <Button onClick={handleTranslate} disabled={!inputText.trim() || isTranslating} className='px-8'>
                        {isTranslating ? (
                          <>
