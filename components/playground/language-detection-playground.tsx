...
// Removed language selectors per spec
import { ModelSelector } from '@/components/playground/model-selector';
import { useToast } from '@/hooks/use-toast';
import { GlowEffect } from '@/components/ui/glow-effect';
import { TextShimmer } from '@/components/ui/text-shimmer';
import { Copy, RotateCcw, SearchCheck } from 'lucide-react';

interface LanguageDetectionPlaygroundProps {
  model: {
    name: string;
    provider: string;
    license: string;
...
export function LanguageDetectionPlayground({
  model,
  selectedModel,
  modelData,
  onModelChange,
}: LanguageDetectionPlaygroundProps) {
  const { toast } = useToast();
  const [inputText, setInputText] = useState('');
  const [detectedText, setDetectedText] = useState<string | null>(null);
  const [isDetecting, setIsDetecting] = useState(false);
  // Language selectors removed per spec
  ...
  // Languages list removed as selectors are not needed
  ...
  const handleDetect = () => {
    ...
    setTimeout(() => {
      ...
      const output = `Detection Result\n\n` +
        `Input Preview:\n${inputText.slice(0, 200)}${inputText.length > 200 ? '…' : ''}\n\n` +
        `Result:\n- Sample detected language (mock).`;
      setDetectedText(output);
      setIsDetecting(false);

      toast({ title: 'Language detected', description: 'Detection complete.' });
    }, 1500);
  };
  ...
  const handleCopy = async () => {
    ...
    toast({ title: 'Copied', description: 'Detected output copied to clipboard.' });
  };
            ...
            <div className='space-y-4'>
              {!detectedText ? (
                <div className='flex flex-col items-center justify-center py-20 text-center'>
                  ...
                  <p className='text-gray-500 text-sm'>Detected language will appear here</p>
                  <p className='text-gray-400 text-xs mt-2'>Enter text below and click "Detect"</p>
                </div>
              ) : (
                <div className='space-y-4'>
                  <div className='flex items-center justify-between'>
                    <h3 className='text-sm font-semibold text-gray-700'>Detected Language</h3>
                ...
                >
                  <div className='space-y-4'>
                    <div className='flex items-start gap-3'>
                      ...
                      <Button onClick={handleDetect} disabled={!inputText.trim() || isDetecting} className='px-8'>
                        {isDetecting ? (
                          <>
