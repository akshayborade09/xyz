...
function AutoScalingGroupsSection() {
  ...
  const vpcOptions = Array.from(new Set(filteredASGs.map(asg => asg.vpc))).map(
    vpc => ({ value: vpc, label: vpc })
  );
  ...
  return (
    <div>
      {showEmptyState ? (
        <Card className='mt-8'>
          <CardContent>
            <EmptyState {...getEmptyStateMessage('asg')} />
          </CardContent>
        </Card>
      ) : (
        <ShadcnDataTable
...
function TemplatesSection() {
  const [selectedTemplate, setSelectedTemplate] =
    useState<AutoScalingTemplate | null>(null);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const { toast } = useToast();
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>
                  {row.isLatest
                    ? 'This is the latest version of the template'
    ...
    switch (activeTab) {
      case 'templates':
        return {
          ...
          description:
            'Create and manage templates for your auto scaling configurations.',
        };
      default:
        return {
          title: 'Auto Scaling Groups',
          description:
            'Create and manage auto scaling groups for your compute resources.',
        };
    }
  };
  ...
  const getHeaderActions = () => {
    switch (activeTab) {
      case 'templates':
        return (
          <CreateButton
            href='/compute/auto-scaling/templates/create'
      ...
      default:
        return (
          <CreateButton
            href='/compute/auto-scaling/create'
