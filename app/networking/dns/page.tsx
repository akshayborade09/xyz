...
export default function HostedZonesPage() {
  ...
  const showEmptyState =
    shouldShowEmptyState() && filteredHostedZones.length === 0;
  ...
  const renderRecordBreakdown = (recordBreakdown: any) => {
    const breakdown = [];
    if (recordBreakdown.A > 0) {
      breakdown.push(`${recordBreakdown.A} A`);
    }
    if (recordBreakdown.AAAA > 0) {
      breakdown.push(`${recordBreakdown.AAAA} AAAA`);
    }
    if (recordBreakdown.CNAME > 0) {
      breakdown.push(`${recordBreakdown.CNAME} CNAME`);
    }
    if (recordBreakdown.NS > 0) {
      breakdown.push(`${recordBreakdown.NS} NS`);
    }
    ...
    },
    {
      ...
      render: (value: number, row: any) => {
        if (value > 0) {
          return (
            <TooltipWrapper
              content={renderRecordBreakdown(row.recordBreakdown)}
            >
        ...
        return (
          <div className='text-muted-foreground leading-5'>{value} records</div>
        );
      },
    },
    {
  ...
  const handleRefresh = () => {
    ...
    console.log(
      '🔄 Refreshing hosted zones data at:',
  ...
  const hostedZoneIcon = (
    ...
    >
      ...
      <text x='65' y='28' textAnchor='middle' fontSize='7' fill='#059669'>
        8
      </text>

      <circle cx='65' cy='75' r='3' fill='#F59E0B' />
      <text x='65' y='68' textAnchor='middle' fontSize='7' fill='#D97706'>
        5
      </text>

      <circle cx='255' cy='35' r='3' fill='#EC4899' />
      <text x='255' y='28' textAnchor='middle' fontSize='7' fill='#BE185D'>
        3
      </text>

      <circle cx='255' cy='75' r='3' fill='#6366F1' />
      <text x='255' y='68' textAnchor='middle' fontSize='7' fill='#4F46E5'>
        12
      </text>
    </svg>
  );
